...
if 'requests' in (ssafy['language']['python']['python standard library']):
                #(ssafy.get('language').get('python').get('python standard library'))
    print(True)
else:
    print(False)
...
pro-gj
"""
for people in ssafy['classes']['gj'].values():
    print(people)
...
"""
f_lists = ssafy['language']['python']['frameworks'].items()
for name, attr in f_lists:
...
print(f'오늘의 당번은 {random.sample(group_E,3)}')



num = random.random() # 0과 1사이의 임의의 소수

print(round(num,2)*100) # 0.8003784774477363 출력
